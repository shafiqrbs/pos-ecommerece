...
class CategoryRepository extends MaterializedPathRepository
{
    ...
    {
        $global = $config->getGlobalOption();
        $categories = $this->createQueryBuilder("node")
            ->where('node.globalOption = :config')
            ->andWhere('node.level = :level')
            ->setParameter('config', $global)
            ->setParameter('level', 1)
            ->orderBy('node.level','ASC')
            ->getQuery()->getResult();
    ...
    function categoryTree($config){
        ...
        $items = array();
        $globalOption = $config->getGlobalOption()->getId();
        $sql = "SELECT id,parent,name FROM categories WHERE  globalOption_id ={$globalOption}  and  permission = 'private' AND level > 2";
    ...
    function categoryInventoryTree($config){
        ...
        $globalOption = $config->getGlobalOption()->getId();
        $sql = "SELECT name FROM categories WHERE  globalOption_id ={$globalOption}  and  permission = 'private' ";
        $stmt = $this->getEntityManager()->getConnection()->prepare($sql);
        $stmt->execute();
        $results =  $stmt->fetchAll();
        return $results;
