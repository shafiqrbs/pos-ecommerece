...
$(document).on( "click", "#btn-refresh", function(e){
    ...
    $('#balance').html("Due "+financial(payment));
    e.preventDefault();
});
...
$(document).on('change', '.updateProductPrice', function() {
    ...
    if(quantity > maxQuantity) {
        alert('This quantity is not valid, enter less or equal '+maxQuantity);
        return false;
    }
...
$(document).on('click', '.invoice-process', function() {
    ...
    var total  = parseInt($('#total').val()  !== '' ? $('#total').val() : 0 );
    ...
    $.ajax({
        ...
        success     : function(response){
            if(process === 'print'){
                ...
                var salesId = response;
                window.open('/app-pos/desktop/'+ salesId +'/pos-print', '_blank');
            }
            resetInvoice();
        }
    });
...
function jsonResult(response) {
    ...
    $('.specialDiscount').html(obj['specialDiscount']);
