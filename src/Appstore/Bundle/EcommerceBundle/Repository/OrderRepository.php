...
class OrderRepository extends EntityRepository
{
    ...
    protected function handleSearchBetween($qb,$data)
    {
            ...
            if (!empty($process) and $process != 'all') {
    ...
    public function orderCounts()
    {
        ...
        $qb->select('COUNT(e.id) as coountId');
        $qb->andWhere("e.process = :group")->setParameter('group', 'created');
        return $qb->getQuery()->getSingleScalarResult();

    }

    public function orderCountByProcess()
    {
        $qb = $this->createQueryBuilder('e');
        $qb->select('COUNT(e.id) as countId','e.process as process');
    ...
    {
        $qb = $this->createQueryBuilder('e');
        $qb->select('count(e.id) as totalOrder','SUM(e.subTotal) as subTotal','SUM(e.total) as total','SUM(e.discount) as discount')
    ...
    public function monthlyOrderDashboard($option,$user)
    {
        $starDate = date("Y-m-01 00:00:00");
        ...
        $qb->select('COUNT(e.id) as totalOrder','SUM(e.subTotal) as subTotal','SUM(e.total) as total','SUM(e.discount) as discount');
            ...
            $qb->andWhere("e.process != :head")->setParameter('head', "Delete");
            $qb->andWhere("e.updated >= :startDate")->setParameter('startDate', $starDate);
    ...
    public function customerOrders($option,$customers)
    {
        ...
        $qb = $this->createQueryBuilder('e');
        $qb->select('c.id as customerId','count(e.id) as totalOrder','SUM(e.subTotal) as subTotal','SUM(e.receive) as receive','SUM(e.total) as total')
            ->join('e.customer','c')
            ->where('e.globalOption = :option') ->setParameter('option', $option)
            ->andWhere("e.process != :head")->setParameter('head', "Delete")
    ...
    public function updateOrder(Order $order)
    {
        ...
        $order->setDiscount($totalAmount - $discountPrice);
        $order->setDiscountAmount($totalAmount - $discountPrice);
        $vat = $this->getCulculationVat($order->getGlobalOption(),$totalAmount);
        $grandTotal = $totalAmount + $order->getShippingCharge() + $vat - $order->getDiscount();
        $order->setVat($vat);
        $order->setTotal($grandTotal);
    ...
    public function updateCustomOrder(Order $order)
    {
        $em = $this->_em;
        $orderItem = $em->getRepository('EcommerceBundle:OrderItem')->getItemOverview($order);
        $totalAmount = $orderItem['totalAmount'];
        $discountPrice = round($orderItem['discountPrice']);
        $totalItem = $orderItem['totalQuantity'];
        $order->setSubTotal($totalAmount);
        $order->setItem($totalItem);
        if($discountPrice > 0){
    ...
    public function insertAndroidOrder(GlobalOption $option , $data= array())
    {
        ...
        $comment        = empty($orderJson['remark']) ? '' : $orderJson['remark'];
        $deliveryDate   = empty($orderJson['deliveryDate']) ? '' : $orderJson['deliveryDate'];
        $timePeriod     = empty($orderJson['timePeriod']) ? '' : $orderJson['timePeriod'];
        ...
        $paymentMode = empty($orderJson['paymentMode']) ? '' : $orderJson['paymentMode'];
        ...
        if(empty($find)){
            ...
            if($paymentMode == "Online-Payment"){
                ...
                $order->setPaymentMode("Mobile");
                $order->setTransactionMethod($method);
                $account = $em->getRepository('AccountingBundle:AccountMobileBank')->findOneBy(array('globalOption'=>$option,'serviceName' => $transactionMethod));
                if($account){
                    $order->setAccountMobileBank($account);
                }
                $order->setPaymentMobile($paymentMobile);
    ...
    public function insertJsonOrderItem(Order $order,$data)
    {
        ...
        foreach ($orderItem as $row){
            ...
            if(empty($find) and !empty($itemId)){
                ...
                $description = (isset($row['description']) and $row['description']) ? $row['description'] :'';
                $orderItem->setDescription($description);
                $quantity = (isset($row['orderedQuantity']) and $row['orderedQuantity'] > 0) ? $row['orderedQuantity'] : 1;
                $price = isset($row['discountPrice']) ? $row['discountPrice'] : $item->getSalesPrice();
                if($item){
                    $orderItem->setItem($item);
                    if($item->getCategory()) {
                    ...
                    $discount = ($item->getDiscountPrice() > 0 ) ? $item->getDiscountPrice() : $item->getSalesPrice();
                    $orderItem->setDiscountPrice($discount);
                    $orderItem->setQuantity($quantity);
                    $orderItem->setSubTotal($item->getSalesPrice() * $quantity);
                    $em->persist($orderItem);
                    $em->flush();
                }
    ...
    public function getApiOrders($option, $arr)
    {
        ...
        foreach ($result as $key => $row){
            ...
            $data[$key]['paymentMode'] = ($row->getPaymentMode()) ? $row->getPaymentMode():'';
            ...
            $data[$key]['deliveryDate'] = $row->getDeliveryDate() ? $row->getDeliveryDate()->format('Y-m-d H:i'):'';
            $data[$key]['deliveryTime'] = $row->getDeliveryDate() ? $row->getDeliveryDate()->format('g:i A'):'';
    ...
    public function getApiOrderDetails($order)
    {
        ...
        $data['paymentMode'] = ($row->getPaymentMode()) ? $row->getPaymentMode():'';
