...
class OrderItemRepository extends EntityRepository
{
    ...
    public function getItemOverview(Order $order){
        ...
        $qb->select('SUM(e.price * e.quantity) AS totalAmount,SUM(e.discountPrice * e.quantity) AS discountPrice,SUM(e.quantity) AS totalQuantity');
    ...
    public function updateSpecialDiscountEcommerceSales(Order $order)
    {
        $em = $this->_em;
        /* @var $item OrderItem */
        foreach ($order->getOrderItems() as $item){
            $discount = (($item->getPrice() * $order->getDiscountCalculation())/100);
            $item->setDiscountPercent($order->getDiscountCalculation());
            $discountPrice = ($item->getPrice() - $discount);
