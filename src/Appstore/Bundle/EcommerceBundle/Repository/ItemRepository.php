...
class ItemRepository extends EntityRepository
{
    ...
    public function insertCopyInventoryItem(\Appstore\Bundle\InventoryBundle\Entity\Item  $copyEntity)
    {
        ...
        if(empty($exist)){
            ...
            $exist->setPurchasePrice($copyEntity->getPurchasePrice());
            $exist->setSalesPrice($copyEntity->getSalesPrice());
            $exist->setDiscountPrice($copyEntity->getDiscountPrice());
        }
        $em->flush();
    ...
    public function handleSearchBetween($qb,$data){
        ...
        if (!empty($name)) {
            $qb->andWhere('item.name LIKE :searchTerm OR item.nameBn LIKE :searchTerm OR category.slug LIKE :searchTerm OR brand.name LIKE :searchTerm  OR promotion.name LIKE :searchTerm');
            $qb->setParameter('searchTerm', '%'.$name.'%');
        }
    ...
    public function getApiProduct(GlobalOption $option,$data = array())
    {
        ...
        if($result){
            foreach($result as $key => $row) {
                ...
                $data[$key]['name']                    = ucfirst($this->stringNullChecker(trim($product)));
                if($productBn){
                    $data[$key]['nameBn']              = $this->stringNullChecker(trim($productBn));
                }else{
                    $data[$key]['nameBn']              = ucfirst($this->stringNullChecker(trim($product)));
                }
                ...
                $data[$key]['category']                 = ucfirst($this->stringNullChecker($row['categoryName']));
                $data[$key]['categoryBn']               = ucfirst($this->stringNullChecker($row['categoryNameBn']));
                $data[$key]['brandId']                  = (int)($row['brandId']);
                $data[$key]['brand']                    = ucfirst($this->stringNullChecker($row['brandName']));
                $data[$key]['brandBn']                  = ucfirst($this->stringNullChecker($row['brandNameBn']));
                $data[$key]['discountId']               = (int)($row['discountId']);
                $data[$key]['discount']                 = ucfirst($this->stringNullChecker($row['discountName']));
                $data[$key]['discountBn']               = ucfirst($this->stringNullChecker($row['discountNameBn']));
    ...
    public function getApiorderItemUpdatePrice(GlobalOption $option,$ids)
    {
        ...
        $qb->leftJoin('item.productUnit','productUnit');
        $qb->leftJoin('item.discount','discount');
        $qb->join('item.inventoryItem','it');
        $qb->select('item.id as id','it.remainingQnt as quantity','item.maxQuantity as maxQuantity');
        $qb->addSelect("CASE WHEN (item.discountPrice IS NOT NULL) THEN item.discountPrice  ELSE  item.salesPrice END  as price");
        $qb->addSelect("CASE WHEN (item.discountPrice IS NOT NULL) THEN item.salesPrice  ELSE 0  END  as discountPrice");
        $qb->where("item.ecommerceConfig = :config")->setParameter('config', $config);
        $qb->andWhere("item.id IN (:ids)")->setParameter('ids',$ids);
        $result = $qb->getQuery()->getArrayResult();
        $data = array();
        if($result){
            foreach($result as $key => $row) {
    ...
    public function copyStockToEcommerce(GlobalOption $option)
    {
        ...
        $medicineConfig = $option->getInventoryConfig()->getId();
    ...
    public function nonExistEcommerecProduct(){

        $qb = $this->_em->createQueryBuilder();
        $qb->from('InventoryBundle:Item','product');
    ...
    {
        $em = $this->_em;
        $rows = $this->nonExistEcommerecProduct();
        foreach ($rows as $row):
            $item = $em->getRepository("InventoryBundle:Item")->find($row['id']);
            $this->insertCopyInventoryItem($item);
            $item->setIsWeb(1);
        endforeach;
        $em->flush();
    ...
    public function searchWebStock($q, EcommerceConfig $config)
    {
        ...
        $query->addSelect('CONCAT(e.name, \' (\',e.nameBn, \')\') AS text');
        $query->where($query->expr()->like("e.name", "'%$q%'"  ));
        $query->andWhere("e.ecommerceConfig = :config")->setParameter('config', $config->getId());
        $query->orWhere($query->expr()->like("e.nameBn", "'%$q%'"));
        $query->groupBy('e.name');
        $query->orderBy('e.name', 'ASC');
        $query->setMaxResults( '5x0' );
