...
class AccountSalesRepository extends EntityRepository
{
	...
	public function reportSalesCustomers($global,$data)
    {
        $month = (isset($data['month']) and $data['month']) ? $data['month']: date('m');
        $year = (isset($data['year']) and $data['year']) ? $data['year']: date('Y');
        $satrtdate = "{$year}-{$month}-01";
        $qb = $this->createQueryBuilder('e');
        $qb->select('COALESCE(SUM(e.totalAmount),0) as totalAmount','COALESCE(SUM(e.amount),0) as amount');
        $qb->addSelect('customer.id as customerId ,customer.name as name ,customer.gender as gender ,customer.employeeId as employeeId, customer.unit as unit, customer.category as category, customer.line as line,  customer.section as section, customer.department as department, customer.profession as profession');
       // $qb->addSelect('CONCAT(customer.name, " ",customer.profession," ", customer.unit," ", customer.line," ,", customer.section) AS details');
        $qb->join('e.customer','customer');
        $qb->where("e.globalOption = :global")->setParameter('global', $global);
        $start = date('Y-m-01 00:00:00',strtotime($satrtdate));
        $qb->andWhere("e.updated >= :startDate")->setParameter('startDate', $start);
        $end = date('Y-m-t 23:59:59',strtotime($satrtdate));
        $qb->andWhere("e.updated <= :endDate")->setParameter('endDate',$end);
        $qb->groupBy('e.customer');
        $qb->orderBy('customer.name', 'ASC');
        $result = $qb->getQuery()->getArrayResult();

        $sql = "SELECT customer_id , DAY(created) as day, COALESCE(SUM(totalAmount),0) as amount 
                FROM account_sales as sales
                WHERE globalOption_id = :globalOption AND sales.process = 'Approved' AND created >= :startDate  AND created <= :endDate 
                GROUP BY customer_id,day ORDER BY sales.balance DESC";

        $qb = $this->getEntityManager()->getConnection()->prepare($sql);
        $qb->bindValue('globalOption',$global->getId());
        $qb->bindValue('startDate', $start);
        $qb->bindValue('endDate', $end);
        $qb->execute();
        $stores = array();
        $daySales =  $qb->fetchAll();
        foreach ($daySales as $sales):
            $key = "{$sales['customer_id']}-{$sales['day']}";
            $stores[$key] = $sales['amount'];
        endforeach;

        $sql1 = "SELECT DAY(created) as day, COALESCE(SUM(totalAmount),0) as amount 
                FROM account_sales as sales
                WHERE globalOption_id = :globalOption AND sales.process = 'Approved' AND created >= :startDate  AND created <= :endDate 
                GROUP BY day ORDER BY sales.balance DESC";

        $qb1 = $this->getEntityManager()->getConnection()->prepare($sql1);
        $qb1->bindValue('globalOption',$global->getId());
        $qb1->bindValue('startDate', $start);
        $qb1->bindValue('endDate', $end);
        $qb1->execute();
        $dalies = array();
        $daydalies =  $qb1->fetchAll();
        foreach ($daydalies as $sales):
            $key = $sales['day'];
            $dalies[$key] = $sales['amount'];
        endforeach;
        return array('customers'=> $result,'daylies' => $dalies,'sales' => $stores);
    ...
    public function customerLedgerOverview($globalOption, $customer)
    {
        $qb = $this->createQueryBuilder('e');
    ...
    public function customerOrderLedger(GlobalOption $globalOption,$data = '')
    {
        $startDate = isset($data['startDateTime'])  ? $data['startDateTime'] : '';
        $endDate =   isset($data['endDateTime'])  ? $data['endDateTime'] : '';
        ...
        if (!empty($startDate) ) {
            $start = date('Y-m-d 00:00:00',strtotime($data['startDateTime']));
            $qb->andWhere("e.updated >= :startDate");
            $qb->setParameter('startDate', $start);
        }
        if (!empty($endDate)) {
            $end = date('Y-m-d 23:59:59',strtotime($data['endDateTime']));
            $qb->andWhere("e.updated <= :endDate");
            $qb->setParameter('endDate',$end);
        }
        $qb->orderBy('e.updated','ASC');
    ...
    public function customerOutstanding($globalOption, $data = array())
    {
        ...
        $mobile = isset($data['mobile']) ? $data['mobile']:'';
        $qb = $this->createQueryBuilder('e');
        $qb->select('COALESCE(SUM(e.totalAmount),0) as totalAmount, COALESCE(SUM(e.amount),0) as amount,(COALESCE(SUM(e.totalAmount),0) - COALESCE(SUM(e.amount),0)) as customerBalance ');
        $qb->addSelect('customer.id as customerId ,customer.employeeId as employeeId ,customer.name as customerName , customer.mobile as customerMobile, customer.section as section, customer.unit as unit, customer.category as category, customer.line as line, customer.department as department, customer.profession as profession');
        $qb->join('e.customer','customer');
    ...
    public function customerOutstandingLedger($globalOption, $customers)
    {
        $cus = array();
        ...
        $qb = $this->createQueryBuilder('e');
        $qb->join('e.customer','customer');
        $qb->select('customer.id as customerId ,(COALESCE(SUM(e.totalAmount),0) - COALESCE(SUM(e.amount),0)) as balance ');
        $qb->where("e.globalOption = :globalOption")->setParameter('globalOption', $globalOption->getId());
        $qb->andWhere("e.process = 'approved'");
        $qb->andWhere("customer.id IN (:customers)")->setParameter('customers', $cus);
        $qb->groupBy('customer.id');
