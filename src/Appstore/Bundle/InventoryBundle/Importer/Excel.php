...
class Excel
{
    ...
    public function import($data)
    {
        ...
        foreach($this->data as $key => $item) {
            ...
            $purchaseItem->setQuantity(($item['Quantity']) ? $item['Quantity'] : 1);
            $purchaseItem->setPurchasePrice(($item['PurchasePrice']) ? $item['PurchasePrice'] : 1);
            $purchaseItem->setDiscountPrice($item['DiscountPrice']);
    ...
    private function getItem($item)
    {
        ...
        if($itemObj == NULL) {
            ...
            if($itemObj == NULL) {
                ...
                $itemObj->setNameBn($this->sentence_case($item['ProductNameBn']));
                $itemObj->setCategoryName($this->sentence_case($item['Category']));
                $itemObj->setCategoryNameBn($this->sentence_case($item['CategoryBn']));
                $itemObj->setBrandName($this->sentence_case($item['Brand']));
                $itemObj->setBrandNameBn($this->sentence_case($item['BrandBn']));
                $itemObj->setSizeName($this->sentence_case($item['Size']));
                $itemObj->setDiscountPrice($item['DiscountPrice']);
    ...
    private function getMasterItem($item)
    {
        ...
        if($masterItem == NULL) {
            ...
            if($masterItem == NULL){
                ...
                $masterItem->setNameBn($item['ProductNameBn']);
    ...
    private function getCategory($item)
    {
        ...
        $global = $this->getInventoryConfig()->getGlobalOption();
        ...
        if($category == NULL) {
            ...
            if($category == null) {
                ...
                $category->setNameBn($item['CategoryBn']);
                $category->setInventoryConfig($this->getInventoryConfig());
                $category->setEcommerceConfig($global->getEcommerceConfig());
