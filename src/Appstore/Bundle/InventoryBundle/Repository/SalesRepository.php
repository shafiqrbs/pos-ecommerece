...
class SalesRepository extends EntityRepository
{
	...
	public function salesCount($id){
	    ...
	    $query = $this->createQueryBuilder('e');
	    $query->select('COUNT(e.id) as lifeTimeOrder');
        $query->where("e.customer = :customer")->setParameter('customer', $id);
    ...
    public function insertPosSales(GlobalOption $option,Pos $pos,$cart)
    {
        ...
        $sales->setSpecialDiscount($pos->getSpecialDiscount());
        ...
        $sales->setDue($sales->getTotal() - $sales->getReceive());
    ...
    public function insertEcommerceSales(Order $order)
    {
        ...
        $sales->setRemark($order->getComment());
        $sales->setProcess("Done");
        $sales->setSalesMode("Self");
        $sales->setPaymentStatus("Paid");
        $em->persist($sales);
        $em->flush();
    ...
    public function insertEcommerceDirectOrder(Order $order)
    {
        ...
        $sales->setDeviceSalesId($order->getInvoice());
        $sales->setSubTotal($order->getSubTotal());
        $sales->setDiscountCalculation($order->getDiscountCalculation());
        $sales->setSpecialDiscount($order->isSpecialDiscount());
        $sales->setDiscount($order->getDiscount());
        $sales->setTotal($order->getTotal());
        $sales->setPayment($order->getReceive());
        $sales->setReceive($order->getReceive());
        ...
        $sales->setUpdated($order->getUpdated());
        $sales->setSalesBy($order->getProcessBy());
        $sales->setApprovedBy($order->getApprovedBy());
        $sales->setProcess("Done");
        $sales->setSalesMode("Self");
        $em->persist($sales);
        $em->flush();
        ...
        if($cart->contents()){
           foreach ($cart->contents() as $item):
               ...
               if ($stockId) {
                   ...
                   $salesItem->setMrp($stockId->getSalesPrice());
                   $salesItem->setPurchasePrice($stockId->getAvgPurchasePrice());
                   if($sales->getDiscountCalculation() > 0 ){
                       $discount = (($stockId->getSalesPrice() * $sales->getDiscountCalculation())/100);
                       $salesItem->setDiscountPercent($sales->getDiscountCalculation());
                       $discountPrice = ($stockId->getSalesPrice() - $discount);
                       $salesItem->setDiscountPrice($discountPrice);
                       $salesItem->setDiscountPercent($sales->getDiscountCalculation());
                   }else{
                       $discount = ($stockId->getDiscountPrice() > 0 ) ? $stockId->getDiscountPrice() : $stockId->getSalesPrice();
                       $salesItem->setDiscountPrice($discount);
                   }
    ...
    public function getApiSales(GlobalOption $terminal, $data = array())
    {
        ...
        $qb->addSelect('c.name as customer','c.employeeId as employeeId');
        $qb->addSelect('s.id as id');
        $qb->addSelect('s.created as created');
        $qb->addSelect('s.process as process');
        $qb->addSelect('s.invoice as invoice');
