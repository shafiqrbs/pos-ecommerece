...
class ItemRepository extends EntityRepository
{
    ...
    public function  getApiStock(GlobalOption $option , $data = '')
    {
        ...
        $qb->leftJoin('m.productUnit','u');
        $qb->select('e.id as stockId','e.barcode as barcode','e.name as name','e.nameBn as nameBn','e.remainingQnt as quantity','e.salesPrice as salesPrice','e.discountPrice as discountPrice','e.salesPrice as price','e.purchaseAvgPrice as purchasePrice','e.path as path');
      ...
      //  $qb->addSelect("CASE WHEN (pi.serialNo IS NOT NULL OR pi.serialNo != '') THEN GROUP_CONCAT(CONCAT(pi.id,'*#*',pi.serialNo,'*#*',pi.quantity))  ELSE  '' END  as serialNos");
        ...
        $qb->groupBy('e.id');
        $qb->orderBy('e.remainingQnt','DESC');
        $result = $qb->getQuery()->getArrayResult();
        $data = array();
  ...
  WHERE medicineConfig_id =:config";
    ...
    public function getExistMasterItem($inventory,$category,$name,$nameBn ='')
    {
        $em = $this->_em;
        $exist = $em->getRepository('InventoryBundle:ItemBrand')->findOneBy(array('inventoryConfig'=>$inventory,'name'=>$name));
        if($exist){
