...
class ItemController extends Controller
{
        ...
        if ($form->isValid()) {
            ...
            if($checkData['count'] != 0 ) {
                ...
                if($brandName){
                    ...
                    $entity->setBrandNameBn($brand->getNameBn());
                }
                ...
                if($entity->getCategory()){
                    $entity->setCategoryName($entity->getCategory()->getName());
                    $entity->setCategoryNameBn($entity->getCategory()->getNameBn());
                }
                if($entity->getSize()){
                    $entity->setSizeName($entity->getSize()->getName());
                }
                if($entity->getCategory()){
                    $entity->setCategoryName($entity->getCategory()->getName());
                    $entity->setCategoryNameBn($entity->getCategory()->getNameBn());
                ...
                $em->persist($entity);
                $em->flush();
                $em->getRepository('InventoryBundle:ItemPriceHistory')->insertPriceItemHistory($entity);
    ...
    {
        $inventory = $this->getUser()->getGlobalOption()->getInventoryConfig();
        $commerce = $this->getUser()->getGlobalOption()->getEcommerceConfig();
        $em = $this->getDoctrine()->getManager();
        $entity = new Item();
        $form   = $this->createCreateForm($entity);
        $items = $this->getDoctrine()->getRepository("InventoryBundle:Product")->getMasterItems($inventory);
        $categories = $this->getDoctrine()->getRepository("ProductProductBundle:Category")->categoryInventoryTree($inventory);
        $brands = $this->getDoctrine()->getRepository("EcommerceBundle:ItemBrand")->brandInventoryTree($commerce);
    ...
    public function updateWebPriceAction(Request $request)
    {
	    ...
	    }elseif($data['name'] == 'DiscountPrice'){
            $entity->setDiscountPrice($data['value']);
            $em->flush();
            $em->getRepository('InventoryBundle:ItemPriceHistory')->insertPriceItemHistory($entity);
            $this->getDoctrine()->getRepository(\Appstore\Bundle\EcommerceBundle\Entity\Item::class)->insertCopyInventoryItem($entity);
        }elseif($data['name'] == 'SalesPrice'){
            $entity->setSalesPrice($data['value']);
            $em->flush();
            $em->getRepository('InventoryBundle:ItemPriceHistory')->insertPriceItemHistory($entity);
            $this->getDoctrine()->getRepository(\Appstore\Bundle\EcommerceBundle\Entity\Item::class)->insertCopyInventoryItem($entity);
        }elseif($data['name'] == 'Category'){
	        $category = $this->getDoctrine()->getRepository(Category::class)->find($data['value']);
            $entity->getMasterItem()->setCategory($category);
            $em->flush();
        }else{
