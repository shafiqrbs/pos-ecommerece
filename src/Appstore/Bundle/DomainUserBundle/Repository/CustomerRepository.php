...
class CustomerRepository extends EntityRepository
{
    ...
    public function getActiveAppsCustomer(GlobalOption $option, $data = [])
    {
        ...
        $qb->join('e.customerUser','cu');
        $qb->select('e.employeeId','e.name','e.gender','e.department','e.line','e.section','e.mobile','e.unit','e.category','e.profession','e.creditLimit');
        $qb->addSelect('cu.updated as created');
        $qb->where("e.globalOption =".$option->getId());
        if (!empty($startDate) ) {
            $start = date('Y-m-d 00:00:00',strtotime($startDate));
            $qb->andWhere("cu.updated >= :startDate");
            $qb->setParameter('startDate', $start);
        }
        if (!empty($endDate)) {
            $end = date('Y-m-d 23:59:59',strtotime($endDate));
            $qb->andWhere("cu.updated <= :endDate");
   ...
   public function resetCustomer($global)
    {

        $qb = $this->_em->createQueryBuilder();
        $q = $qb->update(Customer::class, 's')
            ->set('s.creditLimit', '?1')
            ->set('s.globalOption', '?2')
            ->where('s.globalOption = ?10')
            ->setParameter(1, 0)
            ->setParameter(2, 2)
            ->setParameter(10, $global->getId())
            ->getQuery();
        $q->execute();

         $sqlStockPurchase = "UPDATE Customer c JOIN
(
    SELECT p.creditLimit as PcreditLimit, p.employeeCode
      FROM customer_import p JOIN Customer m
        ON p.employeeCode = m.employeeId
        where  m.globalOption_id = {$global->getId()}
) s
   ON c.employeeId = s.employeeCode
  SET c.creditLimit = REPLACE(PcreditLimit, ',', '')";
    ...
    public function newExistingCustomerForMedicineSales($globalOption,$phone,$data)
    {
        ...
        if(strlen($mobile) > 0){
            ...
            $category = isset($data['customerCategory']) ? $data['customerCategory']:'';
            $entity = $em->getRepository('DomainUserBundle:Customer')->findOneBy(array('globalOption' => $globalOption ,'mobile' => $mobile));
            if($entity){
                $entity->setAddress($address);
    ...
    public function newExistingCustomerForSales($globalOption,$mobile,$data)
    {
        ...
        if($entity){
            $entity->setAddress($address);
            if($email){ $entity->setEmail($email); }
            if($employeeId){ $entity->setEmployeeId($employeeId); }
            $em->flush($entity);
            return $entity;
        }else{
            $entity = new Customer();
            ...
            if($employeeId){
                $entity->setEmployeeId($employeeId);
                $entity->setCustomerId($employeeId);
            }else{
                $entity->setEmployeeId($entity->getCustomerId());
            }
            $entity->setAddress($address);
            if($email){$entity->setEmail($email); }
            $entity->setGlobalOption($globalOption);
    ...
    public function getParticulars($slug)
    {

        $qb = $this->createQueryBuilder('customer');
        if($slug == 'profession'){
            $qb->select('customer.profession as particular');
            $qb->groupBy('customer.profession');
        }elseif($slug == 'section'){
            $qb->select('customer.section as particular');
            $qb->groupBy('customer.section');
        }elseif($slug == 'unit'){
            $qb->select('customer.unit as particular');
            $qb->groupBy('customer.unit');
        }elseif($slug == 'category'){
            $qb->select('customer.category as particular');
            $qb->groupBy('customer.category');
        }elseif($slug == 'department'){
            $qb->select('customer.department as particular');
            $qb->groupBy('customer.department');
        }
    ...
    public function searchAutoComplete(GlobalOption $globalOption, $q, $type = 'NULL')
    {
        ...
        $query->addSelect('CONCAT(e.employeeId, \' - \', e.name, \' (\',e.mobile, \')\') AS text');
    ...
    public function processUser(GlobalOption $option){

       // $customers = $this->findBy(array('globalOption'=>$option,'process'=>'Pending','customerType'=>'customer'));

        $qb = $this->createQueryBuilder('customer');
        $qb->where("customer.globalOption = :globalOption")->setParameter('globalOption', $option);
        $qb->andWhere("customer.name != :name")->setParameter('name', 'Default');
        $qb->andWhere("customer.mobile IS NOT NULL");
        $qb->andWhere("customer.status = 0");
        $qb->andWhere("customer.isNew = 0");
        $qb->andWhere("customer.customerType = 'customer'");
        $qb->orderBy('customer.created','DESC');
        $customers = $qb->getQuery()->getResult();
        foreach ($customers as $customer){
            $em = $this->_em;
            $existUser = $em->getRepository('UserBundle:User')->findOneBy(array('username' => "{$customer->getMobile()}"));
            if(empty($existUser)) {
                ...
                $em3 = $this->_em;
                $customer->setCustomerId($customer->getEmployeeId());
                $customer->setUser($entity->getId());
                $customer->setStatus(true);
                $customer->isNew(true);
                $customer->setProcess('Approved');
                $em3->persist($customer);
                $em3->flush();
            }
        }
        return "success";
    ...
    public function monthlyEmployeeMigrationUser(GlobalOption $option){
    ...
    public function getGenderBaseCustomer(GlobalOption $option)
    {
        $qb = $this->createQueryBuilder('customer');
        $qb->select('customer.gender as name','count(customer.id) as counts');
        $qb->where("customer.globalOption = :globalOption")->setParameter('globalOption', $option);
        $qb->andWhere("customer.name != :name")->setParameter('name', 'Default');
        $qb->andWhere("customer.mobile IS NOT NULL");
        $qb->andWhere("customer.customerType = 'customer'");
        $qb->groupBy('customer.gender');
        $genders = $qb->getQuery()->getArrayResult();

        $qb->select('customer.department  as name','count(customer.id) as counts');
        $qb->where("customer.globalOption = :globalOption")->setParameter('globalOption', $option);
        $qb->andWhere("customer.name != :name")->setParameter('name', 'Default');
        $qb->andWhere("customer.mobile IS NOT NULL");
        $qb->andWhere("customer.customerType = 'customer'");
        ...
        $qb->select('customer.category  as name','count(customer.id) as counts');
        $qb->where("customer.globalOption = :globalOption")->setParameter('globalOption', $option);
        $qb->andWhere("customer.name != :name")->setParameter('name', 'Default');
        $qb->andWhere("customer.mobile IS NOT NULL");
        $qb->andWhere("customer.customerType = 'customer'");
        $qb->groupBy('customer.category');
        $qb->orderBy('name');
        $categorys = $qb->getQuery()->getArrayResult();

        $qb->select('customer.section  as name','count(customer.id) as counts');
        $qb->where("customer.globalOption = :globalOption")->setParameter('globalOption', $option);
        $qb->andWhere("customer.name != :name")->setParameter('name', 'Default');
        $qb->andWhere("customer.mobile IS NOT NULL");
        $qb->andWhere("customer.customerType = 'customer'");
        $qb->groupBy('customer.section');
        $qb->orderBy('name');
        $sections = $qb->getQuery()->getArrayResult();

        $qb->select('customer.profession  as name','count(customer.id) as counts');
        $qb->where("customer.globalOption = :globalOption")->setParameter('globalOption', $option);
        $qb->andWhere("customer.name != :name")->setParameter('name', 'Default');
        $qb->andWhere("customer.mobile IS NOT NULL");
        $qb->andWhere("customer.customerType = 'customer'");
        ...
        $qb->select('customer.unit  as name','count(customer.id) as counts');
        $qb->where("customer.globalOption = :globalOption")->setParameter('globalOption', $option);
        $qb->andWhere("customer.name != :name")->setParameter('name', 'Default');
        $qb->andWhere("customer.mobile IS NOT NULL");
        $qb->andWhere("customer.customerType = 'customer'");
        $qb->groupBy('customer.unit');
        $qb->orderBy('name');
        $units = $qb->getQuery()->getArrayResult();
